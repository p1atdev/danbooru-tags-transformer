...
DAT_PRETRAINED_CONFIG_ARCHIVE_MAP = {
...
class DartConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`DartModel`]. It is used to instantiate a Dart model
    according to the specified arguments, defining the model architecture. Instantiating a configuration with the
    defaults will yield a similar configuration to that of the Dart
    [/]() architecture.
    ...
    ```python
    >>> from transformers import DartConfig, DartModel

    >>> # Initializing a OPT facebook/opt-large style configuration
    >>> configuration = DartConfig()

    >>> # Initializing a model (with random weights) from the facebook/opt-large style configuration
    >>> model = DartModel(configuration)
